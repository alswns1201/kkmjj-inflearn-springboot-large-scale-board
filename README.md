...
#### 인기글 관리 (실시간) - Kafka 도입
인기글 기준 점수  :  좋아요 * 3 + 댓글수 * 2 +  조회수 * 1
매일 오전 1시에 업데이트 되어야함 
-> 매일 오전 1시에 업데이트 되기 위해서 1시간 만에 처리하기엔 대규모에서는 촉박할 수 있다. 
-> 병렬 시스템을 구축 하더라도 인기글 서비스 때문에 타 서비스에 영향이간다 . 
-> 시간적인 제약 , 개발 복잡성 등 배치처리에는 한계가 있음으로 'stream` 처리를 해야한다.
[종류]
- 게시글 삭제/수정/생성
- 댓글 생성/삭제
- 좋아요 생성/삭제
